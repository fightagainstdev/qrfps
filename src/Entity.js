...
export default class Entity{
    ...
    Broadcast(msg){
        ...
        // Also broadcast to parent (EntityManager) only if this is not already from EntityManager
        if(this.parent && this.parent.Broadcast && this.parent.constructor.name !== 'EntityManager'){
            this.parent.Broadcast(msg);
        }
    }

    RemoveFromParent(){
        if(this.parent && this.parent.entities){
            const index = this.parent.entities.indexOf(this);
            if(index !== -1){
                this.parent.entities.splice(index, 1);
                console.log('Entity removed from parent');
            }
        }
    }
