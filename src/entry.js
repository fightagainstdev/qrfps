...
class FPSGameApp{
  ...
  EntitySetup(){
    ...
    // Start with one enemy at a fixed position
    ...
    // Note: We're now using direct score updates instead of events
    ...
    // 注册game_over事件，触发OnGameOver
    this.entityManager.RegisterEventHandler(() => {
      this.OnGameOver();
    }, 'game_over');
  ...
  SpawnInitialEnemy(){
    console.log('Spawning initial enemy at fixed position');

    // Fixed position for the first enemy
    const spawnPosition = new THREE.Vector3(10.8, 0.0, 22.0);

    const npcEntity = new Entity();
    npcEntity.SetPosition(spawnPosition);
    npcEntity.SetName(`Mutant${this.npcCount++}`);
    console.log('Creating enemy entity:', npcEntity.Name);
    ...
    console.log('Enemy entity added to manager');

  }

  SpawnEnemy(){
    console.log('SpawnEnemy called, npcCount:', this.npcCount);

    // Generate positions on the floor, avoiding boxes and existing enemies
    const spawnAreas = [
      // Main floor areas (avoiding box locations)
    ...
    console.log('Enemy spawned successfully on floor');
  }

  OnEnemyDied = () => {
    console.log('OnEnemyDied called, score before:', this.score);
    this.score += 1;
    const uimanagerEntity = this.entityManager.Get("UIManager");
    console.log('uimanagerEntity:', uimanagerEntity);
    const uimanager = uimanagerEntity?.GetComponent("UIManager");
    console.log('uimanager:', uimanager);
    if (uimanager) uimanager.SetScore(this.score);
    else console.warn('UIManager not found!');
    // Update enemy count
    this.UpdateEnemyCount();
    // Every 2 enemies killed = 1 heal charge
    const playerEntity = this.entityManager.Get("Player");
    if(playerEntity){
      const playerHealth = playerEntity.GetComponent("PlayerHealth");
      if(playerHealth && this.score % 2 === 0){
        playerHealth.AddHealCharge();
      }
    }
  ...
  UpdateEnemyCount(){
    // Count living enemies (not in 'dead' state)
    const livingEnemies = this.entityManager.entities.filter(entity => {
      if(entity.Name && entity.Name.startsWith('Mutant') && entity.GetComponent('CharacterController')){
        const controller = entity.GetComponent('CharacterController');
        return controller.stateMachine && controller.stateMachine.currentState &&
               controller.stateMachine.currentState.Name !== 'dead';
      }
      return false;
    }).length;

    const uimanager = this.entityManager.Get("UIManager").GetComponent("UIManager");
    uimanager.SetEnemyCount(livingEnemies);
  }

  OnGameOver = () => {
    console.log('OnGameOver called - game should end now');
    // Stop spawning enemies and show game over
    this.spawnTimer = -999; // Disable spawning
    // 禁用所有怪物AI
    this.entityManager.entities.forEach(entity => {
      if(entity.Name && entity.Name.startsWith('Mutant')){
        const controller = entity.GetComponent('CharacterController');
        if(controller) controller.canMove = false;
      }
      if(entity.Name === 'Player'){
        // 禁用玩家操作
        const controls = entity.GetComponent('PlayerControls');
  ...
  ShowGameOver(){
    ...
    menu.innerHTML = `
      <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;">
        <h1 style="font-size:4em;color:#fff;">游戏结束</h1>
        <p style="font-size:2em;color:#fff;">最终积分: ${this.score}</p>
        <button id="restart_game" style="font-size:2em;padding:0.5em 2em;margin-top:2em;">再来一局</button>
      </div>
    `;
    ...
    document.getElementById('restart_game').onclick = () => {
  ...
  Step(elapsedTime){
    ...
    // Spawn new enemy every 15 seconds (only if game is not over)
